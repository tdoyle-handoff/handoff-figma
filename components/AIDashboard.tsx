...
export function AIDashboard() {
  ...
  const generateAIAnalysis = () => {
    ...
    setAIAnalysis({
      ...
      taskCompletion: tasks.length > 0 ? (completedTasks / tasks.length) * 100 : 0,
      timelineStatus: {
  ...
  const generateCustomInsights = () => {
    ...
    if (data?.targetClosingDate) {
      const daysUntil = Math.ceil((new Date(data.targetClosingDate).getTime() - Date.now()) / (1000 * 60 * 60 * 24));
      if (daysUntil <= 30 && !data.hasLender) {
        insights.push({
          type: 'warning',
          title: 'Urgent: Financing Setup Needed',
          description: 'With only 30 days until closing, you need to secure financing immediately.',
    ...
    if (completion.percentage > 80 && tasks.length === 0) {
      insights.push({
        ...
        description: 'Your profile is complete! I can now generate personalized task lists.',
  ...
  const loadMarketInsights = async () => {
    if (propertyContext.propertyData?.city && propertyContext.propertyData?.state) {
  ...
  const getCategoryIcon = (category: string) => {
    switch (category) {
      case 'financing': return <DollarSign className="w-4 h-4" />;
      case 'setup': return <Home className="w-4 h-4" />;
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center gap-3">
          ...
          <span className="text-lg">AI is analyzing your home buying progress...</span>
        </div>
      </div>
    );
  }
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            <div className="text-center">
              <div className="text-2xl font-bold">{Math.round(aiAnalysis.completionRate)}%</div>
              <div className="text-sm text-muted-foreground">Setup Complete</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold">{Math.round(aiAnalysis.taskCompletion)}%</div>
              <div className="text-sm text-muted-foreground">Tasks Done</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold">{aiAnalysis.timelineStatus.daysUntilClosing}</div>
              <div className="text-sm text-muted-foreground">Days to Close</div>
            </div>
            <div className="text-center">
              <div className="text-2xl font-bold">{aiAnalysis.keyMetrics.blockers}</div>
        ...
        <Card>
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="text-sm text-muted-foreground">Property Setup</div>
                <div className="font-medium">
                  {aiAnalysis.keyMetrics.setupComplete ? 'Complete' : 'In Progress'}
                </div>
              </div>
            </div>
          </CardContent>
          ...
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="font-medium">
                  {aiAnalysis.keyMetrics.hasPreApproval ? 'Pre-approved' : 
                   aiAnalysis.keyMetrics.hasLender ? 'In Progress' : 'Not Started'}
                </div>
              </div>
            </div>
          </CardContent>
          ...
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <div className="font-medium">
                  {aiAnalysis.keyMetrics.completedTasks} of {aiAnalysis.keyMetrics.totalTasks}
                </div>
              </div>
            </div>
          </CardContent>
          ...
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              <div className={`w-10 h-10 rounded-lg flex items-center justify-center ${
                aiAnalysis.timelineStatus.risk === 'low' ? 'bg-green-100' :
                aiAnalysis.timelineStatus.risk === 'medium' ? 'bg-amber-100' : 'bg-red-100'
              }`}>
                <Clock className={`w-5 h-5 ${
                  aiAnalysis.timelineStatus.risk === 'low' ? 'text-green-600' :
                  aiAnalysis.timelineStatus.risk === 'medium' ? 'text-amber-600' : 'text-red-600'
                }`} />
              </div>
              <div>
                <div className="text-sm text-muted-foreground">Timeline Risk</div>
                <div className="font-medium capitalize">{aiAnalysis.timelineStatus.risk}</div>
              </div>
            </div>
          </CardContent>
        </Card>
      ...
      <Card>
        ...
        <CardContent className="space-y-4">
          {recommendations.slice(0, 3).map((rec, index) => (
            <div key={rec.id} className="flex items-start gap-4 p-4 border rounded-lg hover:bg-muted/50 transition-colors">
          ...
          {recommendations.length === 0 && (
            <div className="text-center py-8 text-muted-foreground">
              ...
              <p className="text-sm">Keep up the excellent progress on your home buying journey.</p>
            </div>
          )}
        </CardContent>
      </Card>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
              ...
              <div className="text-center p-3 bg-muted/30 rounded-lg">
                <div className={`text-2xl font-bold capitalize ${
                  ...
                  marketInsights.marketTrend === 'falling' ? 'text-red-600' : 'text-blue-600'
                }`}>
        ...
        <CardContent>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            ...
            <Button
              variant="outline"
              className="h-auto p-4 flex flex-col items-center gap-2"
              onClick={() => ai.sendMessage("Help me understand the home buying process")}
            >
