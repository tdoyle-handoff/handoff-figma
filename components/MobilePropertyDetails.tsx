...
import { useAttomData, formatCurrency, formatSquareFeet, formatLotSize, formatPropertyType, loadCachedAttomProperty } from '../hooks/useAttomData';
...
export function MobilePropertyDetails({ userProfile, setupData, onNavigate, onStartOver, onEditSetup }: MobilePropertyDetailsProps) {
  const [activeTab, setActiveTab] = useState<'overview' | 'details' | 'financials' | 'history' | 'schools' | 'area' | 'risks' | 'settings'>('overview');
  const [attomProperty, setAttomProperty] = useState<AttomProperty | null>(null);
  ...
  const getPropertyData = () => {
    try {
      ...
      const initialSetupData = localStorage.getItem('handoff-initial-setup-data');
      
      console.log('Mobile - PropertySummary - Retrieving data from localStorage:', {
      ...
      if (initialSetupData) {
        try {
          ...
          console.log('Mobile - PropertySummary - Parsed initial setup data:', parsed);
          ...
          if (parsed.propertyAddress) data.propertyAddress = parsed.propertyAddress;
        } catch (error) {
          console.warn('Mobile - PropertySummary - Error parsing initial setup data:', error);
        }
      }
      ...
      if (screeningData) {
        try {
          ...
          console.log('Mobile - PropertySummary - Parsed screening data:', parsed);
          ...
          if (parsed.propertyAddress && parsed.propertyAddress.trim().length > 0) {
            data.propertyAddress = parsed.propertyAddress;
            console.log('Mobile - PropertySummary - Found property address in screening data:', parsed.propertyAddress);
            
            // Try to parse components from the full address if individual components are missing
            if (!data.address || !data.city) {
              const addressParts = parsed.propertyAddress.split(',').map(part => part.trim());
              ...
              if (addressParts.length > 2) {
                ...
                if (stateZipMatch) {
                  ...
                  if (stateZipMatch[2] && !data.zipCode) data.zipCode = stateZipMatch[2];
                }
              }
            }
          }
        } catch (error) {
          console.warn('Mobile - PropertySummary - Error parsing screening data:', error);
        }
      }
      ...
      if (propertyData) {
        try {
          ...
          console.log('Mobile - PropertySummary - Parsed property data:', parsed);
          ...
          console.warn('Mobile - PropertySummary - Error parsing property data:', error);
        }
      }
      ...
      console.log('Mobile - PropertySummary - Final cleaned property data:', cleanedData);
      return cleanedData;
      
    } catch (error) {
      console.warn('Mobile - PropertySummary - Error getting property data:', error);
  ...
  useEffect(() => {
    ...
    if (propertyData.propertyAddress && propertyData.propertyAddress.trim().length > 10) {
    ...
    // Priority 2: Constructed address from components (but only if we have real data)
      ...
      addressToSearch = `${propertyData.address}, ${propertyData.city}, ${propertyData.state} ${propertyData.zipCode}`.trim();
    ...
    if (addressToSearch && addressToSearch.length > 10 && 
      ...
      console.log('Mobile - Starting Attom search for address:', addressToSearch);
      setIsLoadingAttomData(true);
      setAttomError(null);
      searchByAddress(addressToSearch);
    } else {
      console.log('Mobile - Skipping Attom search - no valid address found. Address data:', {
        propertyAddress: propertyData.propertyAddress,
        address: propertyData.address,
        city: propertyData.city,
        constructed: propertyData.address && propertyData.city ? `${propertyData.address}, ${propertyData.city}, ${propertyData.state}` : null
      });
    }
  }, [propertyData.propertyAddress, propertyData.address, propertyData.city, searchByAddress]);
  ...
  const getEnrichedPropertyData = () => {
    ...
    if (attomProperty) {
      return {
        ...
        propertyType: baseData.propertyType || formatPropertyType(attomProperty.property_details.property_type),
        bedrooms: baseData.bedrooms || attomProperty.property_details.bedrooms,
        bathrooms: baseData.bathrooms || attomProperty.property_details.bathrooms,
        sqft: baseData.sqft || attomProperty.property_details.square_feet,
        lotSize: baseData.lotSize || formatLotSize(attomProperty.property_details.lot_size_sqft, attomProperty.property_details.lot_size_acres),
        yearBuilt: baseData.yearBuilt || attomProperty.property_details.year_built,
  ...
  const retryAttomSearch = () => {
    const addressToSearch = propertyData.propertyAddress || `${propertyData.address}, ${propertyData.city}, ${propertyData.state}`;
    if (addressToSearch && addressToSearch.length > 10 && 
        !addressToSearch.includes('123 Oak Street') && 
        !addressToSearch.includes('Riverside Heights')) {
  ...
  const formatRiskScore = (score?: number) => {
    ...
    if (score >= 80) return { score: Math.round(score), level: 'High', color: 'text-green-600' };
    if (score >= 60) return { score: Math.round(score), level: 'Good', color: 'text-blue-600' };
    if (score >= 40) return { score: Math.round(score), level: 'Fair', color: 'text-yellow-600' };
  ...
  const tabs = [
    ...
    { id: 'financials', label: 'Financial', icon: <DollarSign className="w-4 h-4" /> },
  ...
  const hasBasicPropertyInfo = enrichedData.address || enrichedData.city || enrichedData.price || enrichedData.propertyType;
        ...
        <Card>
          ...
          <CardContent>
            <div className="text-center p-8">
              ...
              <h3 className="text-lg font-medium mb-2">Property Information Not Available</h3>
              <p className="text-muted-foreground mb-4">
                Complete your property setup to see detailed information and enhanced features.
              </p>
              <Button onClick={handleEditSetup} className="flex items-center gap-2">
    ...
    <div className="tab-content-mobile space-y-4">
        ...
        <CardContent className="p-4">
            ...
            <div className="flex-1">
              ...
              {(enrichedData.city || enrichedData.state) && (
                <p className="text-sm text-muted-foreground">
                  {[enrichedData.city, enrichedData.state, enrichedData.zipCode].filter(Boolean).join(', ')}
                </p>
              )}
            </div>
          </div>
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="font-semibold">{enrichedData.price || 'Not specified'}</p>
            ...
            <div className="text-right">
              ...
              <p className="font-semibold">{enrichedData.propertyType || 'Not specified'}</p>
            ...
            <div className="space-y-2">
              {enrichedData.closingDate && (
                <div className="flex items-center justify-between p-2 bg-muted/50 rounded">
                  ...
                  <span className="font-medium">{formatDate(enrichedData.closingDate)}</span>
                </div>
              )}
              {enrichedData.inspectionDate && (
                <div className="flex items-center justify-between p-2 bg-muted/50 rounded">
                  <span className="text-sm">Inspection</span>
                  <span className="font-medium">{formatDateTime(enrichedData.inspectionDate)}</span>
                </div>
              )}
            </div>
          </CardContent>
      ...
      {activeTab === 'overview' && (
        <div className="space-y-4">
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-2 gap-4">
                <div className="p-3 bg-muted/50 rounded-lg text-center">
                  <p className="text-2xl font-bold">{enrichedData.bedrooms || attomProperty?.property_details.bedrooms || 'N/A'}</p>
                  <p className="text-sm text-muted-foreground">Bedrooms</p>
                </div>
                <div className="p-3 bg-muted/50 rounded-lg text-center">
                  <p className="text-2xl font-bold">{enrichedData.bathrooms || attomProperty?.property_details.bathrooms || 'N/A'}</p>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  ...
                  <p className="font-semibold">
                    {enrichedData.sqft || attomProperty?.property_details.square_feet ? 
                      formatSquareFeet(enrichedData.sqft || attomProperty?.property_details.square_feet) : 'N/A'}
                  </p>
                </div>
                <div>
                  <p className="text-sm text-muted-foreground">Lot Size</p>
                  <p className="font-semibold">
                    {enrichedData.lotSize || (attomProperty ? formatLotSize(attomProperty.property_details.lot_size_sqft, attomProperty.property_details.lot_size_acres) : 'N/A')}
                  </p>
                </div>
                <div>
                  <p className="text-sm text-muted-foreground">Year Built</p>
                  <p className="font-semibold">{enrichedData.yearBuilt || attomProperty?.property_details.year_built || 'N/A'}</p>
                </div>
                {attomProperty?.property_details.stories && (
                  <div>
                    <p className="text-sm text-muted-foreground">Stories</p>
                    <p className="font-semibold">{attomProperty.property_details.stories}</p>
                  </div>
                )}
              </div>
            </CardContent>
          ...
          {/* Quality Scores */}
          {attomProperty && (attomProperty.risk_factors?.walkability_score || attomProperty.risk_factors?.school_score) && (
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
                  ...
                  {attomProperty.risk_factors?.walkability_score && (
                    <div className="text-center p-3 bg-blue-50 border border-blue-200 rounded-lg">
                      <div className="text-xl font-bold text-blue-700">
                        {formatRiskScore(attomProperty.risk_factors.walkability_score).score}
                      </div>
                      <div className="text-sm text-blue-600">Walk Score</div>
                      <div className="text-xs text-blue-500">
                        {formatRiskScore(attomProperty.risk_factors.walkability_score).level}
                      </div>
                    </div>
                  )}
                  {attomProperty.risk_factors?.school_score && (
                    <div className="text-center p-3 bg-green-50 border border-green-200 rounded-lg">
                      <div className="text-xl font-bold text-green-700">
                        {formatRiskScore(attomProperty.risk_factors.school_score).score}
                      </div>
                      <div className="text-sm text-green-600">School Score</div>
                      <div className="text-xs text-green-500">
                        {formatRiskScore(attomProperty.risk_factors.school_score).level}
                      </div>
                    </div>
                  )}
                </div>
          ...
          {/* Features */}
          {(enrichedData.features && enrichedData.features.length > 0) || attomProperty && (
            <Card>
              <CardHeader>
                <CardTitle className="flex items-center gap-2">
              ...
              <CardContent>
                ...
                {enrichedData.features && enrichedData.features.length > 0 && (
                  <div className="mb-3">
                    <p className="text-sm text-muted-foreground mb-2">Highlighted Features</p>
                    <div className="flex flex-wrap gap-2">
                      {enrichedData.features.map((feature: string, index: number) = (
                ...
                {attomProperty && (
                  <div className="flex flex-wrap gap-2">
                    {attomProperty.property_details.pool && <Badge variant="outline">🏊‍♂️ Pool</Badge>}
                    {attomProperty.property_details.fireplace && <Badge variant="outline">🔥 Fireplace</Badge>}
                    {attomProperty.property_details.central_air && <Badge variant="outline">❄️ Central Air</Badge>}
                    {attomProperty.property_details.basement && <Badge variant="outline">🏠 Basement</Badge>}
                  </div>
                )}
              </CardContent>
            </Card>
            ...
            <CardContent className="space-y-4">
              <div className="grid grid-cols-1 gap-3">
                <div className="flex justify-between py-2 border-b border-border">
                  ...
                  <span className="font-medium">{enrichedData.propertyType || 'Not specified'}</span>
                </div>
                <div className="flex justify-between py-2 border-b border-border">
                  <span className="text-muted-foreground">Year Built</span>
                  <span className="font-medium">{enrichedData.yearBuilt || attomProperty?.property_details.year_built || 'N/A'}</span>
                </div>
                {(attomProperty?.property_details.garage_spaces || enrichedData.garage) && (
                  <div className="flex justify-between py-2 border-b border-border">
                    <span className="text-muted-foreground">Garage Spaces</span>
                    <span className="font-medium">{attomProperty?.property_details.garage_spaces || enrichedData.garage}</span>
                  </div>
                )}
                {(attomProperty?.property_details.parking_spaces || enrichedData.parking) && (
                  <div className="flex justify-between py-2 border-b border-border">
                    <span className="text-muted-foreground">Total Parking</span>
                    <span className="font-medium">{attomProperty?.property_details.parking_spaces || enrichedData.parking}</span>
                  </div>
                )}
                {attomProperty?.property_details.zoning && (
                  <div className="flex justify-between py-2 border-b border-border">
                    <span className="text-muted-foreground">Zoning</span>
                    <span className="font-medium">{attomProperty.property_details.zoning}</span>
                  </div>
                )}
                {attomProperty?.property_details.heating_type && (
                  <div className="flex justify-between py-2 border-b border-border">
                    <span className="text-muted-foreground">Heating</span>
                    <span className="font-medium">{attomProperty.property_details.heating_type}</span>
                  </div>
                )}
                {attomProperty?.property_details.cooling_type && (
                  <div className="flex justify-between py-2 border-b border-border">
                    <span className="text-muted-foreground">Cooling</span>
                    <span className="font-medium">{attomProperty.property_details.cooling_type}</span>
                  </div>
                )}
              </div>
            </CardContent>
                ...
                <Card>
                  ...
                  <CardContent className="space-y-4">
                    <div className="p-3 bg-green-50 border border-green-200 rounded-lg">
                      <p className="text-sm text-green-700">Estimated Market Value</p>
                      <p className="text-xl font-bold text-green-800">
                        {formatCurrency(attomProperty.valuation.estimated_value)}
                      </p>
                      {attomProperty.valuation.confidence_score && (
                        <p className="text-xs text-green-600">
                          Confidence: {Math.round(attomProperty.valuation.confidence_score)}%
                        </p>
                      )}
                    </div>
                    ...
                    <div className="space-y-3">
                      {attomProperty.valuation.market_value && (
                        <div className="flex justify-between py-2 border-b border-border">
                          ...
                          <span className="font-medium">{formatCurrency(attomProperty.valuation.market_value)}</span>
                        </div>
                      )}
                      {attomProperty.valuation.price_per_sqft && (
                        <div className="flex justify-between py-2 border-b border-border">
                          <span className="text-muted-foreground">Price per Sq Ft</span>
                          <span className="font-medium">${Math.round(attomProperty.valuation.price_per_sqft)}</span>
                        </div>
                      )}
                    </div>
                  </CardContent>
                ...
                <CardContent className="space-y-3">
                  {attomProperty.tax_assessment.total_assessed_value && (
                    <div className="flex justify-between py-2 border-b border-border">
                      ...
                      <span className="font-medium">{formatCurrency(attomProperty.tax_assessment.total_assessed_value)}</span>
                    </div>
                  )}
                  {attomProperty.tax_assessment.annual_tax_amount && (
                    <div className="flex justify-between py-2 border-b border-border">
                      <span className="text-muted-foreground">Annual Taxes</span>
                      <span className="font-medium">{formatCurrency(attomProperty.tax_assessment.annual_tax_amount)}</span>
                    </div>
                  )}
                  {attomProperty.neighborhood?.hoa_info?.monthly_fee && (
                    <div className="flex justify-between py-2 border-b border-border">
                      <span className="text-muted-foreground">HOA Fee</span>
                      <span className="font-medium">{formatCurrency(attomProperty.neighborhood.hoa_info.monthly_fee)}/month</span>
                    </div>
                  )}
                </CardContent>
              </Card>
            ...
            <Card>
              <CardContent className="p-4 text-center">
                ...
                <p className="text-muted-foreground">
                  {attomError ? 'Property valuation data is currently unavailable.' : 'Enhanced financial data requires a valid address'}
                </p>
                {attomError && (
                  <Button 
                    variant="outline" 
                ...
                {!attomError && (
                  <p className="text-sm mt-1 text-muted-foreground">Edit your setup to add a property address</p>
                )}
              </CardContent>
            </Card>
          )}
                ...
                <div className="space-y-2">
                  ...
                  {attomProperty && (
                    <div className="text-xs text-muted-foreground">
                      Last updated: {new Date(attomProperty.last_updated).toLocaleDateString()}
                    </div>
                  )}
                </div>
              </div>
        ...
        <Card>
          <CardContent className="p-4 text-center">
            <div className="w-8 h-8 mx-auto mb-2 opacity-50">
              ...
              {activeTab === 'schools' && <GraduationCap className="w-full h-full" />}
