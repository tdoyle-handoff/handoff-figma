...
import { 
  ...
  Database, Settings, TestTube, Code, Server, Key,
  FileText, Bug, ExternalLink, Copy, Eye
} from 'lucide-react';
...
export function AttomAdminDiagnostic() {
  ...
  const [propertyBasicConfig, setPropertyBasicConfig] = useState<EndpointConfig | null>(null);
  ...
  const testPropertyBasicConfig = async (configData: any) => {
    ...
    const propertyBasic = configData.endpoints.find((ep: any) => 
    ...
    if (propertyBasic) {
      ...
      if (propertyBasic.endpoint === '/property/basicprofile') {
        configIssues.push('Endpoint URL missing API version - should be /propertyapi/v1.0.0/property/basicprofile');
        needsFix = true;
      }

      if (!propertyBasic.endpoint.startsWith('/propertyapi/v1.0.0/') && propertyBasic.endpoint.includes('property')) {
        configIssues.push('Endpoint URL may be missing proper API version prefix');
        needsFix = true;
      }
      ...
      if (!propertyBasic.showInPropertyDetails) {
        configIssues.push('Endpoint is not configured to show in property details');
      }

      const status = needsFix ? 'warning' : (configIssues.length > 0 ? 'warning' : 'success');
      ...
      addDiagnostic(
        ...
        { 
          ...
          needsFix: needsFix,
          recommendedEndpoint: needsFix ? '/propertyapi/v1.0.0/property/basicprofile' : null
        }
      );
      return true;
    } else {
      addDiagnostic(
        ...
        { availableEndpoints: configData.endpoints.map((ep: any) => ({ id: ep.id, name: ep.name, endpoint: ep.endpoint })) }
      );
      return false;
    }
  };
      ...
      if (response.ok) {
        ...
        if (result.success) {
          ...
          if (result.status === 401) {
            ...
            recommendation = 'Verify that ATTOM_API_KEY environment variable is set correctly';
          } else if (result.status === 403) {
            errorAnalysis = 'Access forbidden - API key may lack permissions';
            recommendation = 'Verify API key has access to Property Basic Profile endpoint';
          } else if (result.status === 404) {
            if (result.data?.responseText?.includes('path could not be found')) {
              errorAnalysis = 'Endpoint path is incorrect - likely missing API version';
              recommendation = 'Run "Fix Endpoint URLs" to automatically correct the endpoint path';
            } else {
              errorAnalysis = 'Endpoint not found - check endpoint URL';
              recommendation = 'Verify the endpoint URL is correct for Attom API';
            }
            ...
            recommendation = 'This is a temporary issue with the Attom API servers';
          } else if (result.contentType && !result.contentType.includes('json')) {
        ...
        try {
          ...
          errorText = errorResponse.error || errorResponse.details || 'Unknown server error';
        } catch {
      ...
      if (response.ok) {
        ...
        if (result.status === 401) {
          addDiagnostic(
            ...
            { 
              apiResponse: result,
              recommendation: 'Check that ATTOM_API_KEY environment variable is set correctly'
            }
          );
          return false;
        } else if (result.status === 403) {
          ...
          addDiagnostic(
            ...
            { 
              apiResponse: result,
              recommendation: 'Check that ATTOM_API_BASE_URL is set to https://search.onboard-apis.com'
            }
          );
          return false;
        } else if (result.success || (result.status >= 200 && result.status < 300)) {
          addDiagnostic(
            'Attom API Configuration',
            'success',
          ...
          addDiagnostic(
            ...
            { 
              apiResponse: result,
              recommendation: 'API credentials appear valid but endpoint may need attention'
            }
          );
  ...
  const testDefaultConfigInitialization = async () => {
    try {
      ...
      addDiagnostic(
        'Default Config Init',
        'error',
        `Error resetting default configuration: ${error.message}`,
        { error: error.message }
      );
      return false;
    }
  ...
  const testAndFixEndpointUrls = async () => {
    try {
      ...
      if (response.ok) {
        ...
        if (result.fixedEndpoints > 0) {
          addDiagnostic(
            ...
            { 
              result,
              recommendation: 'Endpoint URLs have been automatically corrected to include proper API versions'
            }
          );
          return 'fixed';
        } else {
      ...
      addDiagnostic(
        'Endpoint URL Validation',
        'error',
        `Error validating endpoint URLs: ${error.message}`,
        { error: error.message }
      );
      return false;
    }
  ...
  const testPropertyDisplayConfig = async () => {
    try {
      ...
      if (response.ok) {
        ...
        const propertyBasicInDisplay = displayEndpoints.find((ep: any) => 
          ep.id === 'property_basic_profile' || ep.category === 'property-basic'
        );
  ...
  const runFullDiagnostic = async () => {
    ...
    try {
      ...
      const finalSuccessCount = diagnostics.filter(d => d.status === 'success').length;
      const finalErrorCount = diagnostics.filter(d => d.status === 'error').length;
      ...
      <div className="max-w-4xl mx-auto space-y-6">
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Comprehensive diagnostic tool for troubleshooting property-basic endpoint configuration and admin panel functionality.
            </CardDescription>
          </CardHeader>
          <CardContent>
                  ...
                  <>
                    ...
                    <div className="text-center p-3 bg-muted/50 rounded-lg">
                      <div className="text-lg font-semibold text-orange-600">
                        {adminConfig.endpoints?.filter((ep: any) => ep.isActive).length || 0}
                      </div>
              ...
              <div className="space-y-4">
                <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <Badge variant="secondary">{propertyBasicConfig.method}</Badge>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-muted-foreground">Active:</span>
                    <Badge variant={propertyBasicConfig.isActive ? "default" : "destructive"}>
                      {propertyBasicConfig.isActive ? 'Yes' : 'No'}
                    </Badge>
                  </div>
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-muted-foreground">Display:</span>
                    <Badge variant={propertyBasicConfig.showInPropertyDetails ? "default" : "secondary"}>
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Button
                        size="sm"
                        variant="ghost"
                        onClick={() => copyToClipboard(propertyBasicConfig.endpoint)}
                      >
          ...
          <CardContent>
            {diagnostics.length === 0 ? (
              <div className="text-center py-8 text-muted-foreground">
                ...
                <p>No diagnostics run yet. Click "Run Full Diagnostic" to begin.</p>
              </div>
            ) : (
                        ...
                        <div className="flex items-center justify-between">
                          ...
                          <div className="flex items-center gap-2">
                            <Badge variant={
                              ...
                              diagnostic.status === 'error' ? 'destructive' :
                              diagnostic.status === 'warning' ? 'secondary' : 'outline'
                            }>
                          ...
                          <div className="space-y-2">
                            ...
                            <div className="flex justify-end">
                              <Button
                                size="sm"
                                variant="ghost"
                                onClick={() => copyToClipboard(JSON.stringify(diagnostic.details, null, 2))}
                              >
        ...
        <Card>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <Button
                variant="outline"
                onClick={() => copyToClipboard(JSON.stringify({ projectId, adminEndpoint: getServerUrl('/attom-admin/configurations') }, null, 2))}
                className="justify-start"
              >
            ...
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Button size="sm" variant="ghost" onClick={() => copyToClipboard(projectId)}>
                    <Copy className="w-3 h-3" />
                  </Button>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between">
                <span className="text-sm text-muted-foreground">Server Base URL:</span>
                <div className="flex items-center gap-2">
                  <code className="text-sm bg-muted px-2 py-1 rounded">
                    {getServerUrl('')}
                  </code>
                  <Button size="sm" variant="ghost" onClick={() => copyToClipboard(getServerUrl(''))}>
                    <Copy className="w-3 h-3" />
                  </Button>
                </div>
              </div>
              ...
              <div className="flex items-center justify-between">
                <span className="text-sm text-muted-foreground">Admin Panel URL:</span>
                <div className="flex items-center gap-2">
                  <code className="text-sm bg-muted px-2 py-1 rounded">
                    {window.location.origin}?attom-admin=true
                  </code>
                  <Button size="sm" variant="ghost" onClick={() => copyToClipboard(`${window.location.origin}?attom-admin=true`)}>
                    <Copy className="w-3 h-3" />
                  </Button>
                </div>
              </div>
