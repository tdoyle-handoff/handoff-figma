...
import { ArrowRight, ArrowLeft, Home, DollarSign, Calendar as CalendarIcon, CheckCircle, User, MapPin, Briefcase, Clock, Search, Building, TrendingUp, Eye, ChevronDown, ChevronUp, AlertCircle } from 'lucide-react';
import { useIsMobile } from './ui/use-mobile';
import { cn } from './ui/utils';
import { useAttomData, formatCurrency, formatSquareFeet } from '../hooks/useAttomData';
...
export function InitialPropertySetup({ onComplete, onBack, isEditMode = false, screeningData: propScreeningData, forceAllSections = false }: InitialPropertySetupProps) {
  ...
  const updatePropertyData = useCallback((updates: Partial<PropertyData>) => {
    ...
    if (isEditMode) {
      try {
        localStorage.setItem('handoff-property-data', JSON.stringify(newData));
      } catch (error) {
  ...
  const handleSubmitAll = useCallback(() => {
    ...
    try {
      ...
      if (toSave.attomProperty) {
        localStorage.setItem('handoff-attom-property', JSON.stringify(toSave.attomProperty));
      }
      // Also mirror to screening data for summary fallback
      if (toSave.propertyAddress) {
        const existingScreening = localStorage.getItem('handoff-screening-data');
        let sd: any = {};
        if (existingScreening) {
          try { sd = JSON.parse(existingScreening); } catch {}
        }
  ...
  const canProceed = useCallback(() => {
    ...
    switch (currentStepData.id) {
      case 'property-basic':
        return screeningData?.hasSpecificProperty ? !!propertyData.propertyAddress : true;
      ...
      <div className="max-w-4xl mx-auto">
              ...
              fallback={
                <div className="text-center py-8">
                  <div className="mb-4">
                    ...
                    <h3 className="text-lg font-medium mb-2">Step Unavailable</h3>
        ...
        <div className="flex items-center justify-between">
          ...
          <Button
            ...
            className={isMobile ? 'mobile-button' : ''}
          >
            {currentStep === steps.length - 1 ? (isEditMode ? 'Save Changes' : 'Complete Setup') : 'Next'}
            <ArrowRight className="w-4 h-4 ml-2" />
          </Button>
        </div>
      </div>
...
function DateTimePicker({ 
  ...
  const [open, setOpen] = useState(false);
  const [timeValue, setTimeValue] = useState(date ? formatTimeForInput(date) : "09:00");
...
function PropertyBasicStep({ propertyData, updatePropertyData, screeningData, isMobile, isEditMode }: any) {
  const [attomProperty, setAttomProperty] = useState<AttomProperty | null>(propertyData.attomProperty || null);
  ...
  const handleAddressSelect = (addressComponents: AttomAddressComponents | null) => {
    ...
    <div className="space-y-6">
      ...
      <div className="space-y-2">
        ...
        <input
          ...
          onChange={(e) => updatePropertyData({ propertyPrice: e.target.value })}
        ...
        <div className="flex items-center justify-between">
          ...
          {attomProperty && attomProperty.valuation.estimated_value && (
            <p className="text-xs text-muted-foreground">
              Est. Value: {formatCurrency(attomProperty.valuation.estimated_value)}
            </p>
          )}
        </div>
      </div>
        ...
        <div className="p-4 bg-primary/5 border border-primary/20 rounded-lg">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              ...
              <Badge variant="secondary" className="text-xs">Verified by Attom</Badge>
            </div>
            <Button
              variant="outline"
              size="sm"
            ...
            >
              ...
              {showAnalysis ? <ChevronUp className="w-3 h-3 ml-1" /> : <ChevronDown className="w-3 h-3 ml-1" />}
            </Button>
          </div>
          ...
          <div className="grid grid-cols-2 gap-3 text-sm">
            <div>
              ...
              <p className="font-medium">{attomProperty.property_details.property_type}</p>
            </div>
            {attomProperty.property_details.bedrooms && (
              <div>
                <p className="text-muted-foreground">Bedrooms</p>
                <p className="font-medium">{attomProperty.property_details.bedrooms}</p>
              </div>
            )}
            {attomProperty.property_details.bathrooms && (
              <div>
                <p className="text-muted-foreground">Bathrooms</p>
                <p className="font-medium">{attomProperty.property_details.bathrooms}</p>
              </div>
            )}
            {attomProperty.property_details.square_feet && (
              <div>
                <p className="text-muted-foreground">Square Feet</p>
                <p className="font-medium">{formatSquareFeet(attomProperty.property_details.square_feet)}</p>
              </div>
            )}
            {attomProperty.property_details.year_built && (
              <div>
                <p className="text-muted-foreground">Year Built</p>
                <p className="font-medium">{attomProperty.property_details.year_built}</p>
              </div>
            )}
            {attomProperty.tax_assessment.annual_tax_amount && (
              <div>
                <p className="text-muted-foreground">Annual Taxes</p>
                <p className="font-medium">{formatCurrency(attomProperty.tax_assessment.annual_tax_amount)}</p>
              </div>
            )}
          </div>
...
function ContractDetailsStep({ propertyData, updatePropertyData, isMobile }: any) {
  ...
  const closingDate = propertyData.closingDate ? new Date(propertyData.closingDate) : undefined;
  const inspectionDate = propertyData.inspectionDate ? new Date(propertyData.inspectionDate) : undefined;

  return (
    <div className="space-y-4">
      <div className="space-y-2">
        <label htmlFor="closingDate" className="text-sm font-medium text-foreground">
      ...
      <div className="space-y-2">
        <label htmlFor="earnestMoney" className="text-sm font-medium text-foreground">
...
function FinancialInfoStep({ propertyData, updatePropertyData, screeningData, isMobile }: any) {
  ...
  const isPercentage = (value: string): boolean => {
    return value.includes('%') || (parseFloat(value.replace(/[^0-9.]/g, '')) <= 100 && !value.includes('$') && !value.includes(','));
  };

  // Calculate loan amount automatically
  const calculateLoanAmount = (propertyPrice: string, downPayment: string): string => {
  ...
  const handleDownPaymentChange = (value: string) => {
    ...
    if (propertyData.propertyPrice) {
      const calculatedLoanAmount = calculateLoanAmount(propertyData.propertyPrice, value);
      updatePropertyData({ 
        downPayment: value, 
  ...
  React.useEffect(() => {
    if (propertyData.propertyPrice && propertyData.downPayment) {
      const calculatedLoanAmount = calculateLoanAmount(propertyData.propertyPrice, propertyData.downPayment);
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        <label htmlFor="hoaFees" className="text-sm font-medium text-foreground">
          HOA Fees (Monthly)
        </label>
        <input
          id="hoaFees"
...
function TeamServicesStep({ propertyData, updatePropertyData, isMobile }: any) {
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="space-y-2">
          <label htmlFor="realtor" className="text-sm font-medium text-foreground">
            Real Estate Agent
          </label>
          <input
            id="realtor"
        ...
        <div className="space-y-2">
          <label htmlFor="lender" className="text-sm font-medium text-foreground">
            Lender
          </label>
          <input
            id="lender"
        ...
        <div className="space-y-2">
          <label htmlFor="attorney" className="text-sm font-medium text-foreground">
        ...
        <div className="space-y-2">
          <label htmlFor="inspector" className="text-sm font-medium text-foreground">
            Home Inspector
          </label>
          <input
            id="inspector"
...
function InvestmentDetailsStep({ propertyData, updatePropertyData, isMobile }: any) {
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div className="space-y-2">
          <label htmlFor="rentalIncome" className="text-sm font-medium text-foreground">
          ...
          <input
            ...
            onChange={(e) => updatePropertyData({ rentalIncome: e.target.value })}
            className={`w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary ${
              isMobile ? 'mobile-input' : ''
            }`}
          />
        ...
        <div className="space-y-2">
          <label htmlFor="capRate" className="text-sm font-medium text-foreground">
            Cap Rate
          </label>
          <input
            id="capRate"
      ...
      <div className="space-y-2">
        <label htmlFor="renovationPlans" className="text-sm font-medium text-foreground">
          Renovation Plans
        </label>
        <textarea
          id="renovationPlans"
          ...
          onChange={(e) => updatePropertyData({ renovationPlans: e.target.value })}
          rows={3}
          className={`w-full px-3 py-2 border rounded-lg focus:outline-none focus:ring-2 focus:ring-primary/20 focus:border-primary ${
            isMobile ? 'mobile-input' : ''
          }`}
