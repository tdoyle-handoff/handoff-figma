...
export function useAppEffects({
  ...
  useEffect(() => {
    const { logUrlModes } = require('../utils/urlModes');
    logUrlModes(modes);
  }, [modes]);
  ...
  useEffect(() => {
    if (auth.isAuthenticated && !auth.isLoading) {
      ...
      console.log('Navigation: User staying on current page:', navigation.currentPage);
    }
  }, [auth.isAuthenticated, auth.isLoading, auth.isGuestMode, navigation.currentPage, navigation.navigateTo]);

  // Clear persisted navigation and user data on sign out
  useEffect(() => {
    if (!auth.isAuthenticated && !auth.isLoading) {
      navigation.clearPersistedNavigation();
