...
if (!projectId || !publicAnonKey) {
  ...
  console.error('- publicAnonKey:', publicAnonKey ? '✓' : '❌');
  throw new Error('Supabase configuration incomplete. Check your environment variables.');
}
...
// Helper function to create UserProfile from Supabase auth user (cached)
...
function createUserProfileFromAuth(user: any): UserProfile {
  ...
  // Clean old cache entries (keep only recent 10)
...
// Optimized auth state management with faster initialization
export class AuthStateManager {
  ...
  private async updateUserMetadata(user: any) {
    try {
      ...
      // Update user metadata in Supabase (non-blocking)
